-- ============================================================
-- 0008_menu_publish.sql
-- Dodaje mechanizm "robocze/opublikowane" dla pozycji menu.
-- Klienci widzą tylko opublikowane dania.
-- ============================================================

-- 1. Dodaj kolumnę is_published
ALTER TABLE public.menu_items
  ADD COLUMN IF NOT EXISTS is_published BOOLEAN NOT NULL DEFAULT false;

-- 2. Wszystkie istniejące dania są już widoczne – oznacz jako opublikowane
UPDATE public.menu_items SET is_published = true WHERE is_published = false;

-- 3. Zaktualizuj politykę SELECT:
--    - restauracja / admin widzą wszystko (szkice i opublikowane)
--    - pozostali użytkownicy widzą tylko opublikowane
...
CREATE POLICY menu_items_select_published ON public.menu_items
  ...
  USING (
    is_published = true
    OR (SELECT role FROM public.profiles WHERE id = auth.uid()) IN ('restaurant', 'admin')
  );

-- 4. Zaktualizuj create_order() – sprawdź is_published przed przyjęciem zamówienia
...
BEGIN
  ...
  LOOP
    ...
    SELECT price, max_quantity, is_published INTO v_price, v_max_qty, v_is_published
    ...
    -- Sprawdź czy danie jest opublikowane
    IF NOT v_is_published THEN
      RAISE EXCEPTION 'Danie nie jest dostępne w menu (nieopublikowane).';
    END IF;
